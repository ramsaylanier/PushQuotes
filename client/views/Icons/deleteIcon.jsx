DeleteIcon = <svg className="delete-icon deck-icon icon" x="0px" y="0px" viewBox="0 0 40 40" enable-background="new 0 0 40 40">
				<path d="M35,39.2L35,39.2c-0.6,0-1.1-0.2-1.5-0.6L20,25.1L6.5,38.6c-0.8,0.8-2.1,0.8-2.9,0l-2-1.9c-0.4-0.4-0.7-0.9-0.7-1.5
					c0-0.6,0.2-1.1,0.6-1.5L15,20L1.4,6.5C1,6.1,0.8,5.6,0.8,5.1C0.8,4.5,1,4,1.4,3.6l2.1-2.1c0.8-0.8,2.2-0.8,3,0L20,15L33.4,1.4
					c0.8-0.8,2.2-0.8,3,0l2.1,2.1C39,4,39.2,4.5,39.2,5.1S39,6.1,38.6,6.5L25,20l13.5,13.6c0.4,0.4,0.6,0.9,0.6,1.5
					c0,0.6-0.2,1.1-0.7,1.5l-2,1.9C36.1,39,35.6,39.2,35,39.2z M20,22.2l15,15c0,0,0,0,0,0l2-1.9L22.1,20l15-15L35,2.9l-15,15L5.1,2.9
					L2.9,5l15,15l-15,15.1l2,2L20,22.2z"/>
				</svg>; 

EditIcon = <svg version="1.1" className="edit-icon deck-icon icon"  x="0px" y="0px" viewBox="0 0 40 40" enable-background="new 0 0 40 40">
				<path fill="#000000" strokeWidth="0" d="M39,4.9l-3.7-3.7c-0.4-0.4-1.1-0.4-1.5,0L3,32c-0.1,0.1-0.2,0.2-0.2,0.4l-1.9,5.5c-0.1,0.4,0,0.8,0.2,1.1
				c0.2,0.2,0.5,0.3,0.8,0.3c0.1,0,0.2,0,0.3,0l5.6-1.8c0.2,0,0.3-0.1,0.4-0.3l25-25l0.4-0.4L39,6.4C39.4,6,39.4,5.3,39,4.9z M6.9,35.5
				l-0.3,0.1l-2-2l0.1-0.2L29,9l2.2,2.2L6.9,35.5z M32.6,9.8l-2.2-2.2l1.2-1.2l2.2,2.2L32.6,9.8z"/>
			</svg>;

PlusIcon = <svg version="1.1" className="plus-icon deck-icon icon" x="0px" y="0px" viewBox="0 0 40 40" enable-background="new 0 0 40 40">
			<path d="M21.2,39.4L21.2,39.4l-2.7-0.1c-1.1,0-2-0.9-2-2.1l0-13.9l-13.8,0c-0.6,0-1.1-0.2-1.5-0.6c-0.4-0.4-0.6-0.9-0.6-1.5l0-2.9
				c0-1.1,0.9-2.1,2.1-2.1l13.8,0.1l0-13.8c0-0.6,0.2-1.1,0.6-1.5c0.4-0.4,0.9-0.6,1.5-0.6l2.9,0c0.6,0,1.1,0.2,1.5,0.6
				c0.4,0.4,0.6,0.9,0.6,1.5l-0.1,13.8l14,0.1c0.6,0,1.1,0.2,1.5,0.6c0.4,0.4,0.6,0.9,0.6,1.5l-0.1,2.7c0,1.1-0.9,2-2.1,2l-13.9,0
				l-0.1,13.9C23.3,38.5,22.3,39.4,21.2,39.4z M18.5,21.4l0,15.9l2.7,0.1l0,1v-1c0,0,0,0,0-0.1l0.1-15.9l15.9,0l0.1-2.7l-16.1-0.1
				l0.1-15.9l-2.9-0.1l0,14.9l0,1L2.7,18.4l-0.1,2.9l14.8,0L18.5,21.4z"/>
			</svg>

QuoteIcon = <svg version="1.1" className="quote-icon deck-icon icon" x="0px" y="0px" viewBox="0 0 40 40" enable-background="new 0 0 40 40">
				<path d="M19.1,6.5c4.6,0,8.7,1.3,11.6,3.6c3,2.4,4.6,5.8,4.6,10c0,1.6-0.2,3.1-0.7,4.5c-0.4,1.1-0.3,2.3,0.1,3.3l1.6,3.7l-4.3-0.6
					c-0.3,0-0.6-0.1-0.9-0.1c-1,0-2,0.2-2.9,0.7c-2.6,1.2-5.7,1.9-9,1.9c-7.5,0-16.2-3.5-16.2-13.4c0-4.1,1.6-7.6,4.6-10
					C10.3,7.8,14.5,6.5,19.1,6.5 M19.1,4.5C9,4.5,0.9,10.1,0.9,20.1S9,35.5,19.1,35.5c3.7,0,7.1-0.7,9.9-2.1c0.6-0.3,1.3-0.5,2-0.5
					c0.2,0,0.4,0,0.6,0l6.9,1c0,0,0.1,0,0.1,0c0.4,0,0.7-0.4,0.5-0.8l-2.6-6c-0.3-0.6-0.3-1.3-0.1-1.9c0.5-1.5,0.8-3.3,0.8-5.1
					C37.3,10.1,29.1,4.5,19.1,4.5L19.1,4.5z"/>
			</svg>

FollowerIcon = <svg version="1.1" className="follower-icon deck-icon icon" x="0px" y="0px" viewBox="0 0 40 40" enable-background="new 0 0 40 40">
				<path d="M20,6.7c0.2,0,0.3,0,0.5,0c3.8,0.2,7,3.4,7.1,7.3c0.1,2.1-0.7,4.1-2.2,5.6c-0.6,0.6-0.9,1.4-0.8,2.2
					c0.1,0.8,0.5,1.6,1.1,2.1c2.7,2.1,4.4,5.5,4.4,9.1v0.2H9.9v-0.2c0-3.6,1.6-7,4.4-9.1c0.7-0.5,1.1-1.3,1.1-2.1
					c0.1-0.8-0.2-1.6-0.8-2.2c-1.4-1.4-2.2-3.3-2.2-5.3C12.4,10.1,15.8,6.7,20,6.7 M20,4.7c-5.3,0-9.6,4.3-9.6,9.6c0,2.6,1.1,5,2.8,6.7
					c0.4,0.4,0.3,1-0.1,1.3c-3.1,2.4-5.2,6.3-5.2,10.7v2.2h24.2v-2.2c0-4.4-2-8.3-5.2-10.7c-0.4-0.3-0.5-0.9-0.1-1.3
					c1.8-1.8,2.9-4.3,2.8-7.1c-0.2-4.9-4.1-8.9-9-9.2C20.4,4.7,20.2,4.7,20,4.7L20,4.7z"/>
			</svg>		

PlayIcon = <svg version="1.1" className="play-icon deck-icon icon" x="0px" y="0px" viewBox="0 0 40 40" enable-background="new 0 0 40 40">
				<path fill="#010101" d="M5.9,5.1L31.6,20L5.9,34.9L5.9,5.1 M4.3,1.5c-0.6,0-1,0.5-1,1.3l0,34.4c0,0.8,0.4,1.3,1,1.3
				c0.2,0,0.5-0.1,0.8-0.2L34.9,21c1-0.6,1-1.5,0-2L5.1,1.8C4.8,1.6,4.5,1.5,4.3,1.5L4.3,1.5z"/>
			</svg>	


StopIcon = <svg version="1.1" className="stop-icon deck-icon icon" x="0px" y="0px" viewBox="0 0 40 40" enable-background="new 0 0 40 40">
				<path fill="#010101" d="M37.7,39H2.3C1.6,39,1,38.4,1,37.7V2.3C1,1.6,1.6,1,2.3,1h35.4C38.4,1,39,1.6,39,2.3v35.4
		C39,38.4,38.4,39,37.7,39z M3.5,36.5h32.9V3.5H3.5V36.5z"/>
			</svg>	

BackIcon = <svg version="1.1" className="back-icon deck-icon icon" x="0px" y="0px" viewBox="0 0 40 40" enable-background="new 0 0 40 40">
				<path d="M38.3,20c0,0.7-0.6,1.3-1.3,1.3l-30,0c2.7,1.9,6.2,4.4,8.2,5.8c0.6,0.4,0.7,1.2,0.3,1.8c-0.2,0.3-0.6,0.5-1,0.5
		c-0.3,0-0.5-0.1-0.7-0.2c-4.1-2.9-10.7-7.6-11.4-8c0,0,0,0,0,0c0,0,0,0,0,0C2.2,21,2.1,21,2.1,20.9c0,0-0.1-0.1-0.1-0.1
		c0,0-0.1-0.1-0.1-0.1c-0.1-0.1-0.1-0.2-0.1-0.3c-0.1-0.4,0-0.9,0.3-1.2C2.1,19,2.2,19,2.3,18.9c3.4-2.4,11-7.8,11.7-8.2
		c0.6-0.3,1.4-0.1,1.7,0.6c0.3,0.6,0.1,1.4-0.5,1.7c-0.5,0.3-4.5,3.1-8.2,5.7l30.1,0C37.7,18.7,38.3,19.3,38.3,20z"/>
			</svg>	

MenuIcon = <div className="menu-icon deck-icon icon" x="0px" y="0px" viewBox="0 0 40 40" enable-background="new 0 0 40 40">
				<div className="bar bar-1"></div>
				<div className="bar bar-2"></div>
				<div className="bar bar-3"></div>
			</div>

SearchIcon = <svg version="1.1" className="search-icon nav-icon icon" x="0px" y="0px" viewBox="-379 281 40 40" enable-background="new -379 281 40 40">
				<path d="M-341.1,317.3l-10.7-10.7c2.4-2.6,3.8-6.1,3.8-9.9c0-8.1-6.6-14.6-14.6-14.6c-8.1,0-14.6,6.6-14.6,14.6
				s6.6,14.6,14.6,14.6c3.4,0,6.5-1.1,8.9-3.1l10.7,10.8c0.2,0.2,0.6,0.4,0.9,0.4s0.6-0.1,0.9-0.4
				C-340.6,318.6-340.6,317.8-341.1,317.3z M-354.4,305.7C-354.4,305.7-354.4,305.7-354.4,305.7c-2.2,2-5,3.2-8.2,3.2
				c-6.7,0-12.1-5.4-12.1-12.1s5.4-12.1,12.1-12.1c6.7,0,12.1,5.4,12.1,12.1C-350.4,300.3-351.9,303.5-354.4,305.7z"/>
			</svg>

LogoIcon = <svg version="1.1" className="logo icon" x="0px" y="0px" viewBox="0 0 600 600" enable-background="new 0 0 600 600">
	<path className="logo-quotebox" fill="#F2C71F" d="M540.7,458.8c0,6.7-5.5,12.2-12.2,12.2h-47.9c-6.7,0-12.2,5.5-12.2,12.2l0,73.3c0,6.7-4.5,9-9.9,5.1
		l-116.7-83.5c-5.5-3.9-15.4-7.1-22.1-7.1l-246.2,0c-6.7,0-12.2-5.5-12.2-12.2l0-415.3c0-6.7,5.5-12.2,12.2-12.2l455.1,0
		c6.7,0,12.2,5.5,12.2,12.2L540.7,458.8z"/>
		<path className="logo-letter letter-p" fill="#FFFFFF" d="M280.2,250c0-61.8,43.8-107.1,105.9-107.1S491.9,188.2,491.9,250c0,29.8-10.2,55.6-27.6,74.5l12.6,13
		c3.7,4.2,3.2,10.5-1,14.1l0,0c-9.7,8.1-24.1,7.1-32.5-2.3l-5.2-4.6c-14.9,8.1-32.6,12.4-52.2,12.4
		C323.9,357.1,280.2,311.8,280.2,250z M415.2,318.6l-5.6-6.6c-8.3-9.5-7.1-24,2.6-32l0,0c4.1-3.4,10.2-2.9,13.7,1.1l14.4,16.8
		c9.3-12.7,14.3-29.5,14.3-47.8c0-42.8-27-74.8-68.6-74.8c-41.9,0-68.6,32-68.6,74.8c0,42.5,26.7,74.8,68.6,74.8
		C396.9,324.8,406.5,322.6,415.2,318.6z"/>
		<path className="logo-letter letter-q" fill="#FFFFFF" d="M110.1,349.6V161.3c0-5.2,4.2-9.4,9.4-9.4h81.6c43.5,0,67.7,29.8,67.7,64.9s-24.5,64.9-67.7,64.9h-45.3
		c-5.2,0-9.4,4.2-9.4,9.4v41c0,14.9-12.1,26.9-26.9,26.9h0C114.3,359,110.1,354.8,110.1,349.6z M231.5,216.8
		c0-19.9-14.9-32.9-35.4-32.9h-40.3c-5.2,0-9.4,4.2-9.4,9.4v47c0,5.2,4.2,9.4,9.4,9.4h40.3C216.6,249.7,231.5,236.6,231.5,216.8z"/>
	</svg>